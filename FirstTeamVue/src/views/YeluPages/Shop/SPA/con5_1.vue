...
const ivecoAll = useCampShop();
onMounted(ivecoAll.ivecoProduct);
</script>
<template>
  <div class="">
    <el-container>
        ...
        <section class="py-3">
          <div class="row">
            <div
              ...
              v-for="item in ivecoAll.iveco.IVECO"
              :key="item.商品細項id"
            >
              <card2
                class="cards"
                :image="`${ivecoAll.Images}${item.產品圖片}`"
                :title="item.產品名稱"
                :description="item.單價"
                :content="item.產品說明"
              />
              <el-button
                @click="
                  {
                    ivecoAll.ivecoDialog(item.商品細項id);
                    ivecoAll.dialogvision[9] = true;
                  }
                "
                class="detail"
            ...
            <el-dialog
              v-model="ivecoAll.dialogvision[9]"
              class="dialog"
              title="商品列表"
              width="30%"
              draggable
              ...
              <div class="common-layout">
                <el-container>
                  <el-aside width="50%">
                    <div>
                      <h6 style="color: black; font-weight: bolder">
                        名稱:{{ ivecoAll.ShopproductsInfo.產品名稱 }}
                      </h6>
                    </div>
                    <div>
                      <label class="form-label"
                        >價錢:{{ ivecoAll.ShopproductsInfo.單價 }}</label
                      >
                    </div>
                    ...
                    <div>
                      <label class="form-label"
                        >總價:{{
                          ivecoAll.ShopproductsInfo.單價 * ivecoAll.count
                        }}</label
                      >
                    </div>
                    <div>
                  ...
                  <el-main>
                    <div>
                      <img
                        :src="`${ivecoAll.Images}${ivecoAll.ShopproductsInfo.產品圖片}`"
                        alt="商品圖片"
