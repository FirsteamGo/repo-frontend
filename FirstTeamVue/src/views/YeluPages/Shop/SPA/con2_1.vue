...
const sadoAll = useCampShop();
onMounted(sadoAll.sadomainProduct);
</script>
<template>
  <div class="">
    <el-container>
        ...
        <section class="py-3">
          <div class="row">
            <div
              ...
              v-for="item in sadoAll.sadomain.SADOMAIN"
              :key="item.商品細項id"
            >
              <card2
                class="cards"
                :image="`${sadoAll.Images}${item.產品圖片}`"
                :title="item.產品名稱"
                :description="item.單價"
                :content="item.產品說明"
              />
              <el-button
                @click="
                  {
                    sadoAll.sadomainDialog(item.商品細項id);
                    sadoAll.dialogvision[3] = true;
                  }
                "
                class="detail"
            ...
            <el-dialog
              v-model="sadoAll.dialogvision[3]"
              class="dialog"
              title="商品列表"
              width="30%"
              draggable
              ...
              <div class="common-layout">
                <el-container>
                  <el-aside width="50%">
                    <div>
                      <h6 style="color: black; font-weight: bolder">
                        名稱:{{ sadoAll.ShopproductsInfo.產品名稱 }}
                      </h6>
                    </div>
                    <div>
                      <label class="form-label"
                        >價錢:{{ sadoAll.ShopproductsInfo.單價 }}</label
                      >
                    </div>
                    ...
                    <div>
                      <label class="form-label"
                        >總價:{{
                          sadoAll.ShopproductsInfo.單價 * sadoAll.count
                        }}</label
                      >
                    </div>
                    <div>
                  ...
                  <el-main>
                    <div>
                      <img
                        :src="`${sadoAll.Images}${sadoAll.ShopproductsInfo.產品圖片}`"
                        alt="商品圖片"
