...
const NorthAll = useCampDataAtore();
onMounted(NorthAll.NorthCampGet)
...
//     NorthArea.北區.splice(0, CampD.length, ...CampD)
// }
            ...
            <el-main>
                <section class="py-3">
                    <div class="row">
                        ...
                        <div class="card m-3 col-lg-4 col-sm-6" v-for="item in NorthAll.North.北部">
                            <div
                                style=" height:400px; display: flex; flex-direction: column; justify-content: space-between;">
                                <!-- 卡片 -->
                                <CampTransparentBlogCard :image="`${NorthAll.MVCimages}${item.圖片}`" :title=item.營區名稱
                            ...
                            <div class="container d-flex justify-content-end">
                                <el-button type="info" class="text-lg font-weight-bolder icon-move-right  "
                                    @click="{ NorthAll.NorthDialog(item.營區細項id); NorthAll.dialogVisibleArr[0] = true; }">
                    ...
                    <!-- 彈跳視窗 -->
                    <div>
                        <el-dialog v-model="NorthAll.dialogVisibleArr[0]" title="營區詳細資訊" width="50%" draggable>
                            <div class="modal-header">
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <span class="dialog-footer">
                                <div>
                                    <img :src="`${NorthAll.MVCimages}${NorthAll.CampDialogAll.圖片}`"
                                ...
                                <div>
                                    ...
                                    <label class="form-lable">
                                        <h4>露營形式：<small class="text-muted"> {{ NorthAll.CampDialogAll.項目內容 }}</small></h4>
                                    </label>

                                </div>
                                <div>
                                    <label class="form-lable">
                                        <h4>營區名稱：<small class="text-muted"> {{ NorthAll.CampDialogAll.營區名稱 }}</small></h4>
                                    </label>

                                </div>
                                <div>
                                    <label class="form-lable">
                                        <h4>營區介紹：<small class="text-muted"> {{ NorthAll.CampDialogAll.營區介紹 }}</small></h4>
                                    </label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">pin_drop
                                        </span>地區 | {{ NorthAll.CampDialogAll.地區 }}</label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">pin_drop
                                        </span>縣市 | {{ NorthAll.CampDialogAll.縣市 }}</label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">near_me
                                        </span>營區地址 | {{ NorthAll.CampDialogAll.營區地址 }} </label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">group
                                        </span>容納人數 | {{ NorthAll.CampDialogAll.容納人數 }}人</label>
                                </div>

                                <RouterLink to="/pages/Yelu-pages/Self/stepall/step2/Camp">
                                    <el-button type="warning" class=" text-lg font-weight-bolder m-2"
                                        @click="NorthAll.NorthDialog(item.營區細項id)">
