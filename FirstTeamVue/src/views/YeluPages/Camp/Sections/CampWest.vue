...
const WestAll = useCampDataAtore();
onMounted(WestAll.WestCampGet)
...
//     WestArea.中區.splice(0, CampD.length, ...CampD)
// }
    ...
    <el-container>
        ...
        <section class="py-3">
            <div class="row">
                ...
                <div class="card m-3 col-lg-4 col-sm-6" v-for="item in WestAll.West.中部">
                    <div  style=" height:500px; display: flex; flex-direction: column; justify-content: space-between;">
                        <!-- 卡片 -->
                        <CampTransparentBlogCard :image="`${WestAll.MVCimages}${item.圖片}`" :title=item.營區名稱
                    ...
                    <div class="container d-flex justify-content-end">
                        <el-button type="info" class="text-lg font-weight-bolder icon-move-right  "
                            @click="{ WestAll.WestDialog(item.營區細項id); WestAll.dialogVisibleArr[1] = true; }">
                    ...
                    <!-- 彈跳視窗 -->
                    <div>
                        <el-dialog v-model="WestAll.dialogVisibleArr[1]" title="營區詳細資訊" width="50%" draggable>
                            <div class="modal-header">
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <span class="dialog-footer">
                                <div>
                                    <img :src="`${WestAll.MVCimages}${WestAll.CampDialogAll.圖片}`" style="width: 300px;height: 300;">
                                </div>
                                <div>
                                    <!-- <label class="form-lable">
                                    <h4>露營形式：<small class="text-muted"> {{ 項目內容 }}</small></h4></label> -->
                                    <label class="form-lable">
                                    <h4>露營形式：<small class="text-muted"> {{ WestAll.CampDialogAll.項目內容 }}</small></h4></label>

                                </div>
                                <div>
                                    <label class="form-lable">
                                    <h4>營區名稱：<small class="text-muted"> {{ WestAll.CampDialogAll.營區名稱 }}</small></h4></label>
                                   
                                </div>
                                <div>
                                    <label class="form-lable">
                                    <h4>營區介紹：<small class="text-muted"> {{ WestAll.CampDialogAll.營區介紹 }}</small></h4></label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">pin_drop
                                    </span>地區  |  {{ WestAll.CampDialogAll.地區 }}</label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">pin_drop
                                    </span>縣市  |   {{ WestAll.CampDialogAll.縣市 }}</label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">near_me
                                    </span>營區地址  |   {{ WestAll.CampDialogAll.營區地址 }} </label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">group
                                    </span>容納人數  |   {{ WestAll.CampDialogAll.容納人數 }}人</label>
                                </div>
