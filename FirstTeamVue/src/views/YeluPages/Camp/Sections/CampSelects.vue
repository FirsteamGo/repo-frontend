...
export default {
  data() {
    ...
    const en_regions = reactive(['North', 'West', 'South', 'East']);
    ...
    const styles = reactive(['露營車', '懶人露營(有床)', '懶人露營(無床)', '自搭露營']);
          ...
          <el-card class="box-card">
            ...
            <div class="m-3">
              ...
              <el-select v-model="selectedRegion" @change="filterCities" style="width: 100px;" placeholder="請選擇">
                <el-option v-for="(region, index) in regions" :key="index" :value="region">
                  <router-link :to="`/pages/Yelu-pages/Camp/${en_regions[index]}`" class=" p-8">{{ region }}
                  </router-link>
                </el-option>
              </el-select>
            ...
            <div class="demo-date-picker m-3">
              <div class="block">
                ...
                <p></p>
                <el-date-picker v-model="date" type="daterange" range-separator="To" start-placeholder="開始日期"
                  end-placeholder="結束日期" size="default" style="width: 280px;" />
              </div>
            </div>
