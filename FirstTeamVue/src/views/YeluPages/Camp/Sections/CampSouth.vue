...
const SouthAll = useCampDataAtore();
onMounted(SouthAll.SouthCampGet);
        ...
        <el-container>
            ...
            <el-main>
                <section class="py-3">
                    <div class="row">
                        ...
                        <div class="card m-3 col-lg-4 col-sm-6" v-for="item in SouthAll.South.南部">
                            <div
                                ...
                                <!-- 卡片 -->
                                <CampTransparentBlogCard :image="`${SouthAll.MVCimages}${item.圖片}`" :title=item.營區名稱
                            ...
                            <div class="container d-flex justify-content-end">
                                <el-button type="info" class="text-lg font-weight-bolder icon-move-right  "
                                    @click="{ SouthAll.SouthDialog(item.營區細項id); SouthAll.dialogVisibleArr[2] = true; }">
                    ...
                    <!-- 彈跳視窗 -->
                    <div>
                        <el-dialog v-model="SouthAll.dialogVisibleArr[2]" title="營區詳細資訊" width="50%" draggable>
                            <div class="modal-header">
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <span class="dialog-footer">
                                <div>
                                    <img :src="`${SouthAll.MVCimages}${SouthAll.CampDialogAll.圖片}`"
                                ...
                                <div>
                                    <label class="form-lable">
                                        <h4>營區名稱：<small class="text-muted"> {{ SouthAll.CampDialogAll.營區名稱 }}</small></h4>
                                    </label>

                                </div>
                                <div>
                                    <label class="form-lable">
                                        <h4>營區介紹：<small class="text-muted"> {{ SouthAll.CampDialogAll.營區介紹 }}</small></h4>
                                    </label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">pin_drop
                                        </span>地區 | {{ SouthAll.CampDialogAll.地區 }}</label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">pin_drop
                                        </span>縣市 | {{ SouthAll.CampDialogAll.縣市 }}</label>
                                </div>
                                <div>
                                    <label class="form-lable"><span class="material-icons">near_me
                                        </span>營區地址 | {{ SouthAll.CampDialogAll.營區地址 }} </label>
                                </div>
                                <div>
